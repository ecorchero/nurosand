...
import { clearDoctorAuthed, isDoctorAuthed } from "@/lib/auth";
...
export default function DoctorPatientPage() {
  ...
  // The reports section renders after data loads, so the initial hash jump is a no-op.
  useEffect(() => {
    if (!patient || window.location.hash !== "#reports") return;
  ...
  async function removePatient() {
    ...
    const confirmed = window.confirm(
      `Remove ${patient.name}? This permanently deletes their plans, sessions, wellness data, and reports.`
    );
    if (!confirmed) return;
  ...
  if (!patient) {
    return (
      <DoctorShell onLogOut={() => { clearDoctorAuthed(); router.push("/doctor"); }}>
    ...
    <DoctorShell onLogOut={() => { clearDoctorAuthed(); router.push("/doctor"); }}>
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-4">
          <PatientAvatar name={patient.name} size={64} className="mt-1" src={patient.avatar_url} />
          <div>
            <Link href="/doctor" className="nav-back">
              ← Overview
            </Link>
      ...
      <section id="reports" className="mt-8 scroll-mt-6">
