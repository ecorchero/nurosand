...
def _patient_payload(db, user: User) -> Dict[str, Any]:
    ...
    wellness = db.exec(
        ...
        .where(WellnessSample.patient_id == user.id)
...
def create_patient(body: PatientCreateIn):
    ...
    with get_session() as db:
        ...
        avatar_url = "/images/Elena_pp.jpeg" if first_name == "lucy" else ""
        db.add(
            PatientProfile(
                user_id=user.id,
                doctor_id=doctor_id,
    ...
    with get_session() as db:
        ...
        if profile:
            ...
            if body.date_of_birth is not None:
                profile.date_of_birth = body.date_of_birth.strip()
            if body.emergency_contact is not None:
                profile.emergency_contact = body.emergency_contact.strip()
            db.add(profile)
        db.commit()
        return _patient_payload(db, user)
...
def delete_patient(patient_id: str):
    with get_session() as db:
        ...
        # These demo tables do not use database foreign keys, so remove every
        # patient-owned record explicitly before deleting the account.
        related = (
            (PerformanceSnapshot, PerformanceSnapshot.patient_id),
            (WorkoutSession, WorkoutSession.patient_id),
            (DailyPlan, DailyPlan.patient_id),
            (WeeklyReport, WeeklyReport.patient_id),
            (WellnessSample, WellnessSample.patient_id),
            (EnvironmentCapture, EnvironmentCapture.patient_id),
            (Plan, Plan.patient_id),
            (PatientProfile, PatientProfile.user_id),
        )
        for model, patient_column in related:
            for row in db.exec(select(model).where(patient_column == patient_id)).all():
                db.delete(row)
    ...
    with get_session() as db:
        ...
        if device == "glasses":
            ...
            if body.connected:
                ...
                elif existing.source != "watch":
                    existing.sleep_hours = existing.sleep_hours or 7.5
                    existing.sleep_quality = existing.sleep_quality or 4
                    existing.resting_hr = existing.resting_hr or 61
