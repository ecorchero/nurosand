...
TEMPLATES: List[Dict[str, Any]] = [
    ...
    _t(
        ...
        gif_url="/videos/exercise_demo/tracing-exercise-demo.gif",
    ),
    _t(
    ...
    _t(
        ...
        gif_url="/videos/exercise_demo/cross-crawl-skeleton.gif",
    ),
    _t(
...
def _seed_demo_today(db: Session, patient_id: str) -> None:
    """Hardcoded demo: open plan for a live GIF session first.
    Review with real videos unlocks after the patient completes the session."""
    line_tpl = db.exec(
    ...
    # Replace any existing plan for today so re-seed / upgrade stays clean.
    today = date.today().isoformat()
    for old in db.exec(
        select(DailyPlan).where(DailyPlan.patient_id == patient_id, DailyPlan.date == today)
    ).all():
        db.delete(old)
    db.commit()
    ...
    dp = DailyPlan(
        ...
        date=today,
        exercises=exercises,
        rationale="Demo: live GIF coaching first, then video review after you finish.",
    )
    db.add(dp)
    db.commit()


def _apply_demo_profile(profile: PatientProfile, *, avatar_url: str, phone: str, address: str, dob: str, emergency: str) -> None:
    profile.glasses_connected = True
    profile.glasses_name = "Ray-Ban Meta"
    profile.watch_connected = True
    profile.watch_name = "Apple Watch"
    ...
    profile.daily_exercise_count = 2


DEMO_PATIENTS = {
    "Quentin Tarantino": {
        "focus": ["dexterity", "strength"],
        "notes": "Demo patient — plan covers both video-guided exercises.",
        "avatar_url": "https://randomuser.me/api/portraits/men/52.jpg",
        "phone": "+1 (310) 555-0142",
        "address": "10880 Wilshire Blvd, Los Angeles, CA",
        "dob": "1963-03-27",
        "emergency": "Sally Menke · +1 (310) 555-0199",
    },
    "Lucy Williams": {
        "focus": ["dexterity", "strength"],
        "notes": "Demo patient — same GIF live session and video review flow as Quentin.",
        "avatar_url": "/images/Elena_pp.jpeg",
        "phone": "+1 (415) 555-0188",
        "address": "450 Sutter St, San Francisco, CA",
        "dob": "1988-06-14",
        "emergency": "Tom Williams · +1 (415) 555-0177",
    },
}
...
def seed(force: bool = False) -> None:
    ...
    with Session(engine) as db:
        ...
        if force:
            # Wipe demo tables so force reseeds cleanly (dev only).
            for model in (
                PerformanceSnapshot,
                WorkoutSession,
                DailyPlan,
        ...
        patients_spec = [
            ...
            (
                "Quentin Tarantino",
                DEMO_PATIENTS["Quentin Tarantino"]["focus"],
                DEMO_PATIENTS["Quentin Tarantino"]["notes"],
                2,
            ),
            (
                "Lucy Williams",
                DEMO_PATIENTS["Lucy Williams"]["focus"],
                DEMO_PATIENTS["Lucy Williams"]["notes"],
                2,
            ),
        ]
        ...
        for name, focus, notes, daily_exercise_count in patients_spec:
            ...
            for days_ago in range(1, 5):
                ...
                watch_source = name in DEMO_PATIENTS
                db.add(
                    WellnessSample(
                        patient_id=u.id,
                        date=d.isoformat(),
                        sleep_hours=(8.0 if days_ago == 1 and watch_source else 6.0 + (days_ago % 3) * 0.7),
                        sleep_quality=4 if days_ago == 1 and watch_source else 3 + (days_ago % 2),
                        resting_hr=58 if watch_source else None,
                        source="watch" if watch_source else "manual",
                    )
                )
                sess = WorkoutSession(
                    daily_plan_id="seed",
            ...
            if name in DEMO_PATIENTS:
                _seed_demo_today(db, u.id)


def upgrade_demo_patient(name: str) -> bool:
    """Apply the Quentin-style demo setup to an existing patient by name."""
    demo = DEMO_PATIENTS.get(name)
    if not demo:
        return False
    init_db()
    with Session(engine) as db:
        user = db.exec(select(User).where(User.role == "patient", User.name == name)).first()
        if not user:
            return False
        ...
        if plan:
            plan.focus_tags = demo["focus"]
            plan.notes = demo["notes"]
            db.add(plan)
        else:
            db.add(
                Plan(
                    patient_id=user.id,
                    focus_tags=demo["focus"],
                    notes=demo["notes"],
                    frequency_per_week=5,
                    session_minutes=15,
                )
        ...
        # Refresh recent wellness to watch source if missing.
        for days_ago in range(1, 5):
            d = (date.today() - timedelta(days=days_ago)).isoformat()
            existing = db.exec(
                select(WellnessSample).where(
                    WellnessSample.patient_id == user.id, WellnessSample.date == d
                )
            ).first()
            if existing:
                existing.sleep_hours = 8.0 if days_ago == 1 else existing.sleep_hours
                existing.sleep_quality = 4 if days_ago == 1 else existing.sleep_quality
                existing.resting_hr = 58
                existing.source = "watch"
                db.add(existing)
            else:
                db.add(
                    WellnessSample(
                        patient_id=user.id,
                        date=d,
                        sleep_hours=8.0 if days_ago == 1 else 6.5,
                        sleep_quality=4,
                        resting_hr=58,
                        source="watch",
                    )
